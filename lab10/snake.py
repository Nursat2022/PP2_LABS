...
class Food:
    ...
    def is_green(self):
        #returns true if apple is green
        return self.num == 5
...
pygame.time.set_timer(APPLE, 5000) #disappears after every 5 second
...
while running:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            running = False
        if event.type == APPLE: #after 5 seconds change the pos and redraw
            F.redraw()
